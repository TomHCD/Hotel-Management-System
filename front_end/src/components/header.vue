<template>
    <div class="header">
        ...
        <a-menu v-model="current" mode="horizontal" theme="light">
            ...
            <a-menu-item key="4"  v-if="userInfo.userType=='Manager'">
                <router-link :to="{ name: 'manageUser'}">
                     <a-icon type="user" />账户管理
                </router-link>
            </a-menu-item>

             <a-menu-item key="5"  v-if="userInfo.userType=='Marketer'">
                <router-link :to="{ name: 'marketer'}">
                    <a-icon type="switcher" />网站营销
                </router-link>
            </a-menu-item>
...
export default {
    ...
    $route: {
        handler: function(val, oldVal){
            if (val.name == 'hotelList' || val.name == 'hotelDetail') {
            this.current = ['1']
            }else if(val.name == 'userInfo') {
            this.current = ['2']
            }else if(val.name == 'manageHotel') {
                this.current = ['3']
            }else if(val.name == 'manageUser') {
                this.current = ['4']
            }
            else {
                this.current = ['5']
